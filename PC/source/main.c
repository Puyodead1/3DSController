...
int WINAPI WinMain(HINSTANCE hInst, HINSTANCE hPrevInst, LPSTR lpCmd, int nShow)
{
	...
	if ((settings.dPad == pov) && !(ContPovNumber == 0))
		settings.dPad = cPov;
	...
	if (vJoy && !updateJoystick(iInterface))
	{
		...
		printf("Connected to vJoy device %d\n", iInterface);

	initNetwork();

	char nButtons = GetVJDButtonNumber(iInterface);
	if (vJoy && nButtons < 16)
		printf("Your vJoy has %d buttons, 3DSController supports 16!\n", nButtons);
	...
	while (1)
	{
		...
		case KEYS:
			...
			// 	{ // made a little bit more accurate to the screen size.
			...
			// 		if (currentTouch.x <= 200)
			// 			TouchKey(1, settings.touchLeft);
			// 		else
			// 			TouchKey(0, settings.touchLeft);
			// 		if (currentTouch.x >= 120)
			// 			TouchKey(1, settings.touchRight);
			// 		else
			// 			TouchKey(0, settings.touchRight); // great
			// 	}
			// 	else
			// 	{
			...
			// }
			// else
			// { // If we are not touching, move to center (Like if you release the joystick on a normal controller).
			...
			{
				if (abs(circlePad.x) < settings.mouseSpeed * 3)
					circlePad.x = 0;
				if (abs(circlePad.y) < settings.mouseSpeed * 3)
					circlePad.y = 0;
			...
			{
				if (abs(cStick.x) < settings.mouseSpeed * 3)
					cStick.x = 0;
				if (abs(cStick.y) < settings.mouseSpeed * 3)
					cStick.y = 0;
